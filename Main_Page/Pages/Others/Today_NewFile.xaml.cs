...
namespace Main_Page.Pages
{
    ...
    /// </summary>
    public sealed partial class Today_NewFile : Page
    {
        Items item_;
        public Today_NewFile()
        {
        ...
        {
            //  if (ItemAccess.Cache_Processed.Count == 0)
            //   {
            //      hold_text.Text = "指定された時間帯にファイルがないか、フォルダが追加されていません。";
            //    }
        }
                ...
                case "Picture":
                    {
                        ...
                        frame.Navigate(typeof(Pages.Graphics), item_in);
                        break;
                    }
                case "Video":
                    {
                        Frame frame = Window.Current.Content as Frame;
                        frame.Navigate(typeof(Pages.MediaPage), item_in);
                        break;
                    }
                case "Music":
                    {
                        Frame frame = Window.Current.Content as Frame;
                        frame.Navigate(typeof(Pages.AudioPage), item_in);
                        break;
                    }
                default:
                    {
                        Frame frame = Window.Current.Content as Frame;
                        frame.Navigate(typeof(Pages.ItemPage), item_in);
                        break;
                    }
            }
        }
        ...
        private async void DeleteContentDialog_PrimaryButtonClick(ContentDialog sender, ContentDialogButtonClickEventArgs args)
        {
            if (Source_.SelectionMode == ListViewSelectionMode.Multiple)
            {
                ...
                {
                    Cache_Processed_Today.Remove(item_D);
            ...
            {
                Cache_Processed_Today.Remove(item_);
        ...
        {
            if (Source_.SelectionMode == ListViewSelectionMode.None)
            {
